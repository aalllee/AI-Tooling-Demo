...
# python generate.py 200 easy      data/easy.json
# python generate.py 200 difficult data/difficult.json
# # python validate.py data/easy.json
# # python summarize.py data/easy.json > SUMMARY.md
# ```
# 
# ---
# 
# ## Notes & tips
# 
# - The generator uses Python’s `random`—outputs vary per run. Add a seed at the top of `generate.py` for reproducibility.  
# - If Claude CLI isn’t installed, generation still completes; `explanation`/`reasoned_answer` may be empty.  
# - To increase variety, widen numeric ranges or add new inequality patterns/templates.
